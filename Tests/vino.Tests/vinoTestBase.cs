...
namespace vino.Tests
{
    public abstract class vinoTestBase : AbpIntegratedTestBase<vinoTestModule>
    {
        ...
        protected void UsingDbContext(Action<vinoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<vinoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<vinoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<vinoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<vinoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<vinoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<vinoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<vinoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<vinoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<vinoDbContext, Task<T>> func)
        {
            T result;
